...
FLAMESHOT = FLAMESHOT or "flameshot gui"
MOVE_TO_LEFT_SCREEN = MOVE_TO_LEFT_SCREEN or "xdotool mousemove 0 0"
MOVE_TO_RIGHT_SCREEN = MOVE_TO_RIGHT_SCREEN or "xdotool mousemove 3830 0"

run_once("xcompmgr")
run_once("cmst -m") -- run connman gui here?
...
awful.layout.layouts = WINDOW_LAYOUTS or {
...
-- mem_graph_widget = require('vgavro-widgets.mem_graph')({ font = "DejaVu 10" })
cpu_graph_widget = require('vgavro-widgets.cpu_graph')({ font = "xos4 Terminus 8" })
--  cpu_graph_widget = require('vgavro-widgets.cpu_graph')({ font = "DejaVu 10" })
...
awful.screen.connect_for_each_screen(function(s)
    ...
    awful.tag({ "壱", "弐", "参", "四" }, s, awful.layout.layouts[1])
...
globalkeys = gears.table.join(
    ...
    -- Standard program
    awful.key({ modkey,           }, "Return", function () awful.spawn(SHELL) end,
    ...
    -- Custom keys
    awful.key({ modkey }, "-", function () awful.util.spawn(SYSTEM_MONITOR) end),
    awful.key({ modkey }, "0", function () awful.util.spawn(MUSIC_PLAYER) end),
    awful.key({ modkey }, "=", function () awful.util.spawn(BROWSER) end),
    awful.key({}, "Print", function () awful.util.spawn(FLAMESHOT) end),
    -- awful.key({ modkey, "Shift" }, "Left", function () awful.util.spawn(MOVE_TO_LEFT_SCREEN) end),
    -- awful.key({ modkey, "Shift" }, "Right", function () awful.util.spawn(MOVE_TO_RIGHT_SCREEN) end),
    -- Multimedia keys
...
clientkeys = gears.table.join(
    ...
    awful.key({ modkey }, "Next", function (c) c.opacity = c.opacity - 0.1 end),
    awful.key({ modkey }, "Prior", function (c) c.opacity = c.opacity + 0.1 end)
)
...
awful.rules.rules = {
    ...
    { rule = { },
      properties = { border_width = beautiful.border_width,
                     ...
                     size_hints_honor = WINDOW_SIZE_HINTS_HONOR,
     },
      callback = WINDOW_SLAVE_DEFAULT and awful.client.setslave or nil
    },
    ...
    { rule_any = {
        ...
        name = {
          --"Event Tester",  -- xev.
          "Crack Attack!", -- yaourt -S crack-attack
        },
        role = {
    ...
    { rule_any = {type = { "normal", "dialog" }
      }, properties = { titlebars_enabled = WINDOW_TITLEBARS_ENABLED }
    },
...
client.connect_signal("focus", function(c)
  ...
  c.opacity = beautiful.opacity_focus or 1
end)
client.connect_signal("unfocus", function(c)
  c.border_color = beautiful.border_normal or ''
  c.opacity = beautiful.opacity_normal or 1
end)
-- }}}
